...
const useForm = (details)=>{
            ...
            if(details.id){
                const fetchDetails = async()=>{
                    ...
                    try{
                        ...
                        if(res.status==='ok') setUser({...user,details:{...user.details,[details.formType]:res.details}})
    ...
    const saveDetails = async(data)=>{
        setAwaiting(true)
        const res = await sendDetails({id:user._id,details:data,type:details.formType})
        if(res.status==='ok') setUser({...user,details:{...user.details,[details.formType]:res.details}})
        else setErr(res.message)
        setAwaiting(false)
    }
